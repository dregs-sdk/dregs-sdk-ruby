...
does the HMAC comparison in constant time, with `OpenSSL.secure_compare`, and rejects stale payloads
as replays. Verifying a re-serialized hash instead of the received bytes will not match, and working
around that by skipping verification would leave the endpoint open to anyone who learns its URL. In
Rails, read `request.raw_post` rather than `params`.
